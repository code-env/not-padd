    ...
    <div className="space-y-4 p-6 bg-gray-900 rounded-lg">
      ...
      {activeOrganization && (
        <div>
          ...
          <div className="p-3 bg-gray-800 rounded">
            ...
            <p><strong>Logo:</strong> {activeOrganization.logo || "No logo"}</p>
          </div>
        </div>
      )}
      ...
      {activeMember && (
        <div>
          ...
          <div className="p-3 bg-gray-800 rounded">
            <p><strong>Role:</strong> {activeMember.role}</p>
            <div className="flex gap-2 mt-2">
              {isOwner && <span className="px-2 py-1 bg-red-600 rounded text-sm">Owner</span>}
              {isAdmin && <span className="px-2 py-1 bg-blue-600 rounded text-sm">Admin</span>}
              {isMember && <span className="px-2 py-1 bg-green-600 rounded text-sm">Member</span>}
            </div>
          </div>
        </div>
      )}
